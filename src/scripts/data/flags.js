/**
 * @file flags.js
 * @description Maps ISO country codes to corresponding SVG flag icons.
 * Provides a helper function to generate HTML for flag display.
 *
 * Imports all SVG flags manually and stores them in `flagMap` using ISO 3166-1 alpha-2 codes.
 * If a flag is missing or cannot be determined, a fallback icon is used.
 * Used for visualizing GitHub user location flags in the UI.
 *
 * @requires ../../assets/flags/ - Collection of country flag SVG files
 * @requires ../../assets/images/geo_code_icon.svg - Fallback icon
 */

import geoCodeIcon from '../../assets/images/geo_code_icon.svg'; // fallback icon

import gb from '../../assets/flags/gb-eng.svg';
...
/**
 * A mapping between lowercase ISO country codes and their corresponding flag image modules
 */
...
/**
 * Return a flag image HTML string based on location/country code.
 * @param {string} location – Free-form text or country code
 * @param {Function} extractCountryCode – A helper that extracts country code (you can inject or import)
 * @returns {string} – HTML string for flag image
 */
export function getFlagHtml(location, extractCountryCode) {
  ...
  return `<img src="${flagSrc}" alt="Flag of ${
    countryCode || 'Unknown'
